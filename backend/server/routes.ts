...
export async function registerRoutes(app: Express): Promise<Server> {
  app.get("/api/auth/verify", authenticateToken, async (req: Request, res: Response) => {
    const authReq = req as AuthenticatedRequest;
    try {
      console.log('Auth verification request for user:', authReq.user.userId);
      const user = await storage.getUser(authReq.user.userId);
      if (!user) {
        console.log('User not found in database:', authReq.user.userId);
  ...
  // Customer routes
  app.get("/api/customers", authenticateToken, requireRole(['frontdesk']), async (req: Request, res: Response) => {
    try {
  ...
  app.post("/api/customers", authenticateToken, requireRole(['frontdesk']), async (req: Request, res: Response) => {
    try {
  ...
  app.get("/api/tickets", authenticateToken, async (req: Request, res: Response) => {
    try {
      ...
      if (authReq.user.role === 'technician') {
        tickets = await storage.getTicketsByTechnician(authReq.user.userId);
      } else {
  ...
  app.get("/api/tickets/:ticketId", authenticateToken, async (req: Request, res: Response) => {
    try {
  ...
  app.post("/api/tickets", authenticateToken, requireRole(['frontdesk']), async (req: Request, res: Response) => {
    try {
  ...
  app.patch("/api/tickets/:ticketId/status", authenticateToken, async (req: Request, res: Response) => {
    try {
      const authReq = req as AuthenticatedRequest;
      ...
      const updatedTicket = await storage.updateTicketStatus(
        req.params.ticketId,
        { ...updateData, technicianId: authReq.user.userId }
      );
  ...
  app.get("/api/stats", authenticateToken, async (req: Request, res: Response) => {
    try {
      ...
      // Role-specific stats
      if (authReq.user.role === 'technician') {
        const myTickets = allTickets.filter(t => t.assignedTechnician === authReq.user.userId);
    ...
    // Reports routes
  app.get("/api/reports", authenticateToken, requireRole(['frontdesk']), async (req: Request, res: Response) => {
    try {
      const allTickets = await storage.getAllTickets();
      ...
      const thisMonthRevenue = allTickets
        .filter(t => t.finalCost && t.serviceStatus === 'Delivered' && 
                (t.completedAt || t.createdAt) && new Date(t.completedAt || t.createdAt!) >= startOfMonth)
        .reduce((sum, t) => sum + parseFloat(t.finalCost || '0'), 0);
      const thisWeekRevenue = allTickets
        .filter(t => t.finalCost && t.serviceStatus === 'Delivered' && 
                (t.completedAt || t.createdAt) && new Date(t.completedAt || t.createdAt!) >= startOfWeek)
        .reduce((sum, t) => sum + parseFloat(t.finalCost || '0'), 0);
      ...
      const avgResolutionTime = allTickets
        ...
        }, 0) / allTickets.filter(t => t.completedAt && t.createdAt).length || 0;
      const reports = {
        summary: {
          totalTickets: allTickets.length,
  ...
  app.post("/api/communication/send", authenticateToken, requireRole(['frontdesk']), async (req: Request, res: Response) => {
    try {
